...
export const FloatingNav = ({
  ...
  const [visible, setVisible] = useState(true); // Set default to true for initial visibility
  const [isMenuOpen, setIsMenuOpen] = useState(false);

  useEffect(() => {
    setVisible(true); // Ensure the navbar is visible on load
  }, []);
  ...
  useMotionValueEvent(scrollYProgress, "change", (current) => {
    if (typeof current === "number") {
      ...
      if (scrollYProgress.get() < 0.05) {
        setVisible(true); // Keep visible if near the top of the page
      } else {
        if (direction < 0) {
          setVisible(true); // Show navbar on upward scroll
        } else {
          setVisible(false); // Hide navbar on downward scroll
        }
      }
    }
  });
  ...
  return (
      <AnimatePresence mode="wait">
        <motion.div
            ...
            animate={{
              y: visible ? 0 : -100, // Slide up when not visible
              opacity: visible ? 1 : 0,
            }}
            transition={{
              duration: 0.2,
